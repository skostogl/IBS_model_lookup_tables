...
for current_study in range(n_studies):
  ...
  ! rf voltage
  V0={float(config["V0max"][current_study])*1e-3};
  ...
  tt:=0.0;
  Npb={config["bunch_intensity"][current_study]};
  Npb0={config["bunch_intensity"][current_study]};
  exi={config["emit_x"][current_study]}*exi;
  eyi={config["emit_y"][current_study]}*eyi;
  vrf400=V0;
